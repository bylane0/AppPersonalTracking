    ...
    public partial class FrmSalaryList : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (detail.SalaryID == 0)
                MessageBox.Show("Seleccione un sueldo de la tabla");
            else
            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if(txtSalary.Text.Trim()!="")
               if(rbMore.Checked)
                ...
                {
                    MessageBox.Show("Seleccione que desea hacer con el sueldo");
                }
        ...
        {
            DialogResult result = MessageBox.Show("Estás seguro que quieres borrar este salario?", "Warning", MessageBoxButtons.YesNo);
            if (result == DialogResult.Yes)
            {
                SalaryBLL.DeleteSalary(detail.SalaryID);
                MessageBox.Show("El salario fue eliminado!");
                FillAllData();
                CleanFilters();
            }
        }
