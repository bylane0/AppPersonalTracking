    ...
    public partial class FrmPermissionList : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (detail.PermissionID == 0)
                MessageBox.Show("No puedes actualizar un permiso que no existe");
            else if (detail.State == PermissionStates.Approved || detail.State == PermissionStates.Disapproved)
                MessageBox.Show("No puedes actualizar un permiso aprobado y/o desaprobado");
            else
            {
                FrmPermission frm = new FrmPermission();
                frm.isUpdate = true;
        ...
        private void FrmPermissionList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns[14].HeaderText = "Explain";
        ...
        {
            DialogResult result = MessageBox.Show("Estás seguro que quieres borrar este permiso?", "Warning", MessageBoxButtons.YesNo);
            if (result == DialogResult.Yes)
            {
                if (detail.State == PermissionStates.Approved || detail.State == PermissionStates.Disapproved)
                    MessageBox.Show("No puedes borrar permisos ya evaluados");
                else
                {
                    PermissionBLL.DeletePermission(detail.PermissionID);
                    MessageBox.Show("El permiso fue borrado!");
                    FillAllData();
                    CleanFilters();
